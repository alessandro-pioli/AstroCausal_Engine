...
> [!TIP]
> **Tip rapido sui media incorporati**: se un video inline non si carica o appare rotto, di solito basta un refresh della pagina (F5) per risolvere.
...
- **Orbita eccentrica e *plunge***: L'eccentricità misura quanto un'orbita legata si discosta dal cerchio perfetto ( $e = 0$ ): al crescere di $e$ , pericentro (punto più vicino) e apocentro (punto più lontano) si divaricano sempre di più. Il *plunge* è la caduta (quasi) diretta sul corpo centrale, con momento angolare (quasi) nullo: è il limite estremo $e \to 1$ delle orbite legate, l'ellisse degenerata in un segmento. In questo senso il plunge, nei casi estremi, è la forma più eccentrica possibile di traiettoria legata.
...
- **BBH e BNS**: Acronimi standard per le due famiglie di binarie compatte: *Binary Black Hole* (coppia di buchi neri, es. GW150914) e *Binary Neutron Star* (coppia di stelle di neutroni, es. GW170817).
...
- **Inspiral**: La lunga fase di avvicinamento a spirale di una coppia binaria compatta: la radiazione gravitazionale sottrae energia orbitale e i due corpi si stringono su orbite sempre più rapide, facendo salire gradualmente frequenza e ampiezza del segnale. È la fase che precede la coalescenza, e il chirp ne è la firma spettrale.
- **Coalescenza / Merger**: L'atto conclusivo dell'inspiral: i due corpi entrano in contatto e si fondono in un unico oggetto. Nel simulatore coincide con l'evento di collisione che assorbe il corpo minore nel maggiore; ciò che nella realtà segue (il *ringdown*) non è modellato ([§8.6](#86-il-troncamento-netto-dello-strain-lassenza-del-ringdown)).
...
7. [La matematica delle heatmap](#7-la-matematica-delle-heatmap)
   ...
   - 7.9 Il Pannello di Telemetria Orbitale (HUD)
...
Matematicamente, questo doppio passo è equivalente a una singola iterazione di Picard sull'equazione del cono di luce, e per orbite ordinarie ( $v \ll c$ ) converge in un colpo. Per regimi estremi ( $v \to c$ ), [§5.1](#51-il-tempo-di-volo-per-sorgenti-in-moto-rettilineo-formula-chiusa) fornisce la soluzione **analitica in forma chiusa**, ricavata da una quadratica nel tempo di volo, che il motore usa al posto della doppia lettura.
...
> Lo scenario "Approccio a c" di [§5.3](#53-showcase-approccio-a-c) sfrutta esattamente il bypass appena descritto: l'ART ignora questo freno di proposito e spinge il Sole oltre $c$ , un caso limite tecnico dichiaratamente impossibile, non un risultato fisico.
...
Il confronto contro lo strain reale H1 ha comunque un limite metodologico dichiarato: il dato negli ultimissimi 50 ms è rumoroso, e la finestra robusta si ferma a $\tau \in [-1{,}0, -0{,}2]$ s. Per quantificare cosa succede *oltre* quella finestra serve un riferimento privo di rumore strumentale: il confronto diretto contro una waveform di relatività numerica BNS è in **[§6.6.1](#661-lo-scenario-bns-gw170817-peters-vs-relatività-numerica-sxs)**. Per lo scenario BBH il confronto con la relatività numerica SXS è documentato in [§6.6.2](#662-lo-scenario-bbh-gw150914-confronto-con-la-relatività-numerica-sxs).
...
Il confronto contro lo strain reale H1 ([§6.6](#66-le-prove-confronto-col-dato-reale)) ha un limite dichiarato: il dato è rumoroso oltre la finestra robusta, e per quantificare cosa succede oltre serve una waveform di relatività numerica. Qui va messo in chiaro un fatto: **nel catalogo SXS una simulazione mirata su GW170817 non esiste** (il catalogo pubblico contiene due sole waveform BNS, entrambe configurazioni generiche; waveform NR mirate sull'evento esistono in altri database, come CoRe, al prezzo però di dover scegliere e dichiarare un'equazione di stato). Si è quindi presa la più vicina disponibile, **SXS:NSNS:0001**: una BNS a masse uguali e non rotanti ( $m_1 = m_2 = 1{,}4\ M_\odot$ source frame), con massa chirp a circa il 2,9% da quella di GW170817 ( $1{,}231$ contro $1{,}197\ M_\odot$ detector frame). Il confronto è costruito di conseguenza: la curva di Peters è calcolata sulla massa chirp di **questa** configurazione, non su quella ufficiale dell'evento, e viene messa contro la **sua** relatività numerica: si misura cioè quanto l'ordine dominante si scosta dalla NR in un sistema BNS quasi identico al nostro. L'ipotesi di lavoro, dichiarata, è che il risultato si trasferisca: dato che il disallineamento di massa chirp vale un sistematico di appena l'1,7% sulla frequenza ( $f \propto \mathcal{M}^{-5/8}$ ), se la waveform SXS di GW170817 esistesse, il confronto reale mostrerebbe con ogni probabilità uno scostamento molto simile a quello riportato qui sotto.
...
|---|---|---|---|---|
| **BNS** (GW170817) | SXS:NSNS:0001 | ~0,97% | **18,80%** | ~18-19% |
...
2. **Stress mareale e disgregazione** *(dalla saturazione cromatica e dalla deformazione geometrica)*. La mappa rivela le condizioni di rottura secondo due regimi fisicamente distinti (il confronto visivo immediato è l'esempio di **Mercurio al perielio**, prima immagine a fine sezione):
