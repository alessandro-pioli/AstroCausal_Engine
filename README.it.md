...
La scelta architetturale fondamentale del progetto è operare in uno spaziotempo **2+1D** (due dimensioni spaziali più il tempo come asse esplicito) su sfondo euclideo piatto, senza risolvere le equazioni di campo della Relatività Generale. Le leggi restano però quelle della fisica tridimensionale reale ( $1/r^2$ , non quello $1/r$ di una gravità intrinsecamente bidimensionale) e il tempo è assoluto nel senso preciso del tempo coordinato di un **osservatore lontano** dal sistema, la stessa convenzione con cui si cronometrano nella realtà pulsar binarie e onde gravitazionali. Questa semplificazione geometrica consente al motore di girare **in tempo reale su qualsiasi PC consumer** stressando la CPU per calcolare la fisica in virgola mobile a doppia precisione (`float64`) tramite parallelizzazione JIT.
...
Nei merger binari compatti, l'orbita decade a causa dell'emissione di onde gravitazionali. Il motore implementa l'accelerazione dissipativa al primo ordine non conservativo (**reazione alla radiazione di ordine 2.5PN**) secondo la formulazione relativistica reale di **Damour-Deruelle** per l'accelerazione relativa $\vec a_{rel}$ (contesto teorico e storia dell'implementazione in [§6.2-6.5 della Guida Fisica](PHYSICS_AND_SCENARIO_GUIDE.it.md#62-cosa-sono-gli-ordini-post-newtoniani-e-il-25pn)):
...
5. **Correzione relativistica dell'inerzia**:
 ...
 $$\vec a_{eff}(t + \Delta t) = \vec{a}(t + \Delta t) \cdot \sqrt{1 - \frac{v^2}{c^2}}$$
...
6. **Secondo "Half-Kick" delle velocità**:
 $$\vec{v}(t + \Delta t) = \vec{v}\left(t + \frac{\Delta t}{2}\right) + \frac{1}{2} \vec a_{eff}(t + \Delta t) \Delta t$$
