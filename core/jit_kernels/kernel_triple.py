...
def update_step_triple_par(
    ...
    hist_L1, heads_L1, mask_L1, len_L1,
    hist_L2, heads_L2, mask_L2, len_L2,
    ...
    for step_idx in range(steps):
        ...
        for i in range(n_bodies):
            ...
            write_to_L1 = (head_0 & STRIDE_L1_MASK) == 0
            write_to_L2 = (head_0 & STRIDE_L2_MASK) == 0
            
            head_1 = heads_L1[i]
            head_2 = heads_L2[i]

            if write_to_L1:
                head_1 = (head_1 + 1) & mask_L1
                heads_L1[i] = head_1
                
            if write_to_L2:
                head_2 = (head_2 + 1) & mask_L2
                heads_L2[i] = head_2

            # 2. GESTIONE MORTE
            if (flags_arr[i] & FLAG_DYING) != 0:
                hist_L0[i, head_0, 0] = VOID_VAL
                if write_to_L1: hist_L1[i, head_1, 0] = VOID_VAL
                if write_to_L2: hist_L2[i, head_2, 0] = VOID_VAL
                continue 
            ...
            if write_to_L1:
                hist_L1[i, head_1, 0] = pos_arr[i, 0]
                hist_L1[i, head_1, 1] = pos_arr[i, 1]
                hist_L1[i, head_1, 2] = vel_arr[i, 0]
                hist_L1[i, head_1, 3] = vel_arr[i, 1]
                hist_L1[i, head_1, 4] = mass_arr[i]

            if write_to_L2:
                hist_L2[i, head_2, 0] = pos_arr[i, 0]
                hist_L2[i, head_2, 1] = pos_arr[i, 1]
                hist_L2[i, head_2, 2] = vel_arr[i, 0]
                hist_L2[i, head_2, 3] = vel_arr[i, 1]
                hist_L2[i, head_2, 4] = mass_arr[i]
            ...
            for idx_j in range(num_active):
                ...
                if ticks_est < len_L0:
                    ...
                    if old_x > VOID_VAL: 
                        ...
                        found_est = True
                elif (ticks_est >> STRIDE_L1_SHIFT) < len_L1:
                    delay_L1 = ticks_est >> STRIDE_L1_SHIFT
                    if True:
                        idx = (heads_L1[j] - delay_L1) & mask_L1
                        old_x = hist_L1[j, idx, 0]
                        if old_x > VOID_VAL: 
                            old_y = hist_L1[j, idx, 1]
                            found_est = True
                elif (ticks_est >> STRIDE_L2_SHIFT) < len_L2:
                    delay_L2 = ticks_est >> STRIDE_L2_SHIFT
                    if True:
                        idx = (heads_L2[j] - delay_L2) & mask_L2
                        old_x = hist_L2[j, idx, 0]
                        if old_x > VOID_VAL: 
                            old_y = hist_L2[j, idx, 1]
                            found_est = True
                ...
                if found_est:
                    ...
                    if true_ticks < len_L0:
                        ...
                        if final_x > VOID_VAL: 
                            ...
                            found_final = True
                    elif (true_ticks >> STRIDE_L1_SHIFT) < len_L1:
                        delay_L1 = true_ticks >> STRIDE_L1_SHIFT
                        if True:
                            idx = (heads_L1[j] - delay_L1) & mask_L1
                            final_x = hist_L1[j, idx, 0]
                            if final_x > VOID_VAL: 
                                final_y, final_vx, final_vy, final_mass = hist_L1[j, idx, 1], hist_L1[j, idx, 2], hist_L1[j, idx, 3], hist_L1[j, idx, 4]
                                
                                dt_L1 = dt * (1 << STRIDE_L1_SHIFT)
                                if delay_L1 > 0:
                                    idx_future = (heads_L1[j] - (delay_L1 - 1)) & mask_L1
                                    fut_x = hist_L1[j, idx_future, 0]
                                    if fut_x > VOID_VAL:
                                        final_ax = (hist_L1[j, idx_future, 2] - final_vx) / dt_L1
                                        final_ay = (hist_L1[j, idx_future, 3] - final_vy) / dt_L1
                                elif delay_L1 + 1 < len_L1:
                                    idx_past = (heads_L1[j] - (delay_L1 + 1)) & mask_L1
                                    past_x = hist_L1[j, idx_past, 0]
                                    if past_x > VOID_VAL:
                                        final_ax = (final_vx - hist_L1[j, idx_past, 2]) / dt_L1
                                        final_ay = (final_vy - hist_L1[j, idx_past, 3]) / dt_L1
                                        
                                found_final = True
                    elif (true_ticks >> STRIDE_L2_SHIFT) < len_L2:
                        delay_L2 = true_ticks >> STRIDE_L2_SHIFT
                        if True:
                            idx = (heads_L2[j] - delay_L2) & mask_L2
                            final_x = hist_L2[j, idx, 0]
                            if final_x > VOID_VAL: 
                                final_y, final_vx, final_vy, final_mass= hist_L2[j, idx, 1], hist_L2[j, idx, 2], hist_L2[j, idx, 3], hist_L2[j, idx, 4]
                                
                                dt_L2 = dt * (1 << STRIDE_L2_SHIFT)
                                if delay_L2 > 0:
                                    idx_future = (heads_L2[j] - (delay_L2 - 1)) & mask_L2
                                    fut_x = hist_L2[j, idx_future, 0]
                                    if fut_x > VOID_VAL:
                                        final_ax = (hist_L2[j, idx_future, 2] - final_vx) / dt_L2
                                        final_ay = (hist_L2[j, idx_future, 3] - final_vy) / dt_L2
                                elif delay_L2 + 1 < len_L2:
                                    idx_past = (heads_L2[j] - (delay_L2 + 1)) & mask_L2
                                    past_x = hist_L2[j, idx_past, 0]
                                    if past_x > VOID_VAL:
                                        final_ax = (final_vx - hist_L2[j, idx_past, 2]) / dt_L2
                                        final_ay = (final_vy - hist_L2[j, idx_past, 3]) / dt_L2
...
def update_step_triple_seq(
    ...
    for step_idx in range(steps):
        ...
        for i in range(n_bodies):
            ...
            write_to_L1 = (head_0 & STRIDE_L1_MASK) == 0
            write_to_L2 = (head_0 & STRIDE_L2_MASK) == 0
            head_1, head_2 = heads_L1[i], heads_L2[i]
            if write_to_L1:
                head_1 = (head_1 + 1) & mask_L1
                heads_L1[i] = head_1
            if write_to_L2:
            ...
            if write_to_L1:
                hist_L1[i, head_1, 0], hist_L1[i, head_1, 1] = pos_arr[i, 0], pos_arr[i, 1]
                hist_L1[i, head_1, 2], hist_L1[i, head_1, 3] = vel_arr[i, 0], vel_arr[i, 1]
                hist_L1[i, head_1, 4] = mass_arr[i]
            if write_to_L2:
                hist_L2[i, head_2, 0], hist_L2[i, head_2, 1] = pos_arr[i, 0], pos_arr[i, 1]
                hist_L2[i, head_2, 2], hist_L2[i, head_2, 3] = vel_arr[i, 0], vel_arr[i, 1]
                hist_L2[i, head_2, 4] = mass_arr[i]
