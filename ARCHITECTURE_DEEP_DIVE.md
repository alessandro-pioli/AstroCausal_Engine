...
Il loop del **[Velocity Verlet (§4.1 della Guida alla Fisica)](PHYSICS_AND_SCENARIO_GUIDE.md#41-lo-schema-di-integrazione)** (lo schema d'integrazione numerica che fa avanzare le orbite passo dopo passo) per la fisica dei corpi ha fasi distinte, ciascuna con un profilo di costo e di parallelismo diverso:
...
```mermaid
flowchart TD
    N["engine.py valuta N: numero di<br/>corpi dello scenario<br/>(una volta sola, all'init e a ogni<br/>rebuild)"]
    N -->|"N maggiore di 35"| T1["self.tick punta alla variante<br/>parallela"]
    N -->|"N minore o uguale a 35"| T2["self.tick punta alla variante<br/>sequenziale"]

    T1 --> P1["Fase 1: O(N)<br/>range, barriera read-after-write"] --> P2["Fase 2: O(N²)<br/>prange, il collo di bottiglia<br/>parallelizzato"] --> P3["Fase 2.5: O(N)<br/>accodata nello stesso prange di<br/>Fase 2"]

    T2 --> S1["Fase 1: O(N)<br/>range, barriera read-after-write"] --> S2["Fase 2: O(N²)<br/>range, con N piccolo l'overhead<br/>dei thread supererebbe il guadagno"] --> S3["Fase 2.5: O(N)<br/>range"]
```
...
```mermaid
flowchart TD
    A["engine.py: refresh_kernel()<br/>eseguito una volta, all'init e a<br/>ogni rebuild"] --> B["Criterio 1: footprint del buffer<br/>causale vs Cache L3 (appena<br/>descritto)"]
    A --> C["Criterio 2: numero di corpi N (§1)"]
    ...
    KERNEL --> H["kernel_helper_inline.py<br/>stesse formule fisiche per ogni<br/>variante, inline sempre attivo"]
    KERNEL --> T["self.tick assegnato al kernel<br/>concreto scelto<br/>a runtime: chiamata diretta, zero<br/>if di selezione"]
    ESEC --> T
```
...
Le heatmap sono le mappe di campo sovrapposte allo spazio simulato (potenziale, marea, strain: il catalogo completo, dal lato fisico, è nel [§7 della Guida alla Fisica](PHYSICS_AND_SCENARIO_GUIDE.md#7-la-matematica-delle-heatmap)). Sono anche, di gran lunga, il carico computazionale più oneroso dell'intera parte grafica, per un motivo strutturale. Una heatmap non ricolora dati già calcolati dalla fisica: campiona il campo da zero, punto per punto, a ogni frame. Il costo è il prodotto di tre fattori:
...
flowchart TD
    ...
    T4["rimozione certificata dal GC (§6)"] --> R

    R["rebuild_simulation()<br/>core/simulation_manager.py"] --> F1

    F1["1. Snapshot<br/>stato dei corpi, copia di storici,<br/>scie e sonda,<br/>timeline di morte/nascita per ogni<br/>corpo"] --> F2
    F2["2. Params<br/>nuovi DT, raggio e reciproci<br/>precalcolati"] --> F3
    F3["3. Planning<br/>matrice SINGLE/DOUBLE/TRIPLE<br/>(§2.1),<br/>pool ridimensionato sui corpi da<br/>ricreare"] --> F4
    F4["4. Wipe &amp; Alloc<br/>array rifatti da zero, nuovo<br/>VOID_VAL,<br/>guardia OOM con dialogo di errore"] --> F5
    F5{"5. Restore<br/>DT e taglie dei buffer<br/>invariati?"}
    F5 -->|"sì: SMART COPY"| C1["storia ricopiata identica,<br/>reindicizzata sul pool compattato"]
    F5 -->|"no: RICOSTRUZIONE"| C2["storico riscritto a ritroso a<br/>velocità costante,<br/>VOID_VAL prima della nascita e<br/>dopo la morte"]
    C1 --> F6
    C2 --> F6
    F6["6-9. Fasi ancillari<br/>indici attivi e LOD, radar<br/>relativistico,<br/>TOP_ATTRACTOR (§3.4), priming del<br/>Verlet"]
```
...
```mermaid
flowchart TD
    A["Per ogni pixel (x, y), in<br/>parallelo su prange(width)"] --> B["Per ogni corpo attivo in p_idx"]
    B --> C["Calcolo del contributo fisico<br/>(potenziale, dΦ/dt, tidal,<br/>quadrupolo... secondo la heatmap)"]
    C --> D["Somma dei contributi di tutti i<br/>corpi attivi"]
    D --> E["Normalizzazione via reciproci<br/>precalcolati, zero divisioni"]
    E --> F["Conversione in colore, scrittura<br/>diretta come uint8 nella texture"]
    F --> G["Buffer restituito al<br/>graphic_renderer per la stampa a<br/>schermo"]
```

> [!IMPORTANT]
> Questo capitolo tratta le heatmap solo dal lato ingegneristico: kernel, risoluzione, FPS. Cosa *sono* fisicamente, cosa mostrano e come si leggono a schermo è il tema del [§7 della Guida alla Fisica](PHYSICS_AND_SCENARIO_GUIDE.md#7-la-matematica-delle-heatmap). A chi non le avesse mai viste si raccomanda di dare almeno uno sguardo superficiale prima di proseguire, perché tutto quello che segue dà per scontato il loro significato.
...
- Serve recuperare velocità senza pagare in FPS? Il tasto `Y` raddoppia il DT: a parità di costo per tick, ogni raddoppio raddoppia la velocità del tempo simulato. Il prezzo qui è l'[errore di troncamento](PHYSICS_AND_SCENARIO_GUIDE.md#42-errore-di-troncamento) dell'integratore, che sulle scale planetarie resta trascurabile per parecchi raddoppi.
...
L'utente può agire su questo bilanciamento in ogni momento coi tasti `T`, `Y`, `G` e i numerici `1`-`5`. La guida operativa, con la tabella di recupero FPS, resta nella [sezione sulle prestazioni del README](README.md#limiti-del-modello-e-gestione-delle-prestazioni).
...
```mermaid
flowchart TD
    A["Stima range [min, max] di<br/>potenziale/energia nella vista<br/>corrente"] --> B["Per ogni pixel nella griglia<br/>campionata:<br/>Calcola il contributo<br/>gravitazionale di tutti gli N<br/>corpi"]
    B --> C["Normalizzazione del valore (0 → 1)<br/>rispetto al range"]
    C --> D["Conversione del potenziale in<br/>colore (RGB lookup)"]
    D --> E["Salvataggio del colore nella<br/>matrice texture"]
    E --> F["Restituzione del buffer al<br/>graphic_renderer per la stampa a<br/>schermo"]
```
...
Qui un fisico sarebbe arrivato subito alla risposta; l'autore ci è arrivato passo passo, ragionando sull'intorno matematico di $\Phi$ diviso l'intorno del tempo, cioè la derivata parziale $\partial\Phi/\partial t$ in ogni punto dello spazio (la lettura fisica della mappa risultante è nel [§7.2 della Guida alla Fisica](PHYSICS_AND_SCENARIO_GUIDE.md#72-variazione-temporale-dφdt)). Da questo percorso a tratti empirico sono stati formulati la struttura e il metodo per tutte le altre [heatmap del campo](PHYSICS_AND_SCENARIO_GUIDE.md#7-la-matematica-delle-heatmap).
...
```mermaid
flowchart TD
    A["Per ogni pixel (x, y) nella<br/>griglia campionata:<br/>Per ogni corpo j attivo:"] --> B["Calcola la posizione passata di j<br/>rispetto a (x, y) [buffer causale]"]
    B --> C["Calcola la velocità radiale di j<br/>verso il pixel (x, y)"]
    C --> D["Calcola il contributo_j = G × M_j<br/>× v_radiale_j / r²"]
    D --> E["Somma i contributi di tutti i<br/>corpi per ottenere dΦ/dt totale"]
    E --> F["Normalizzazione logaritmica<br/>(modulata dal fader in ±ordini di<br/>grandezza)"]
    F --> G["Conversione in colore (scala<br/>divergente: blu = compressione,<br/>rosso = espansione)"]
```
...
**Heatmap GW Strain (quadrupolo proiettato).** L'ultima arrivata della famiglia e quella che spinge più a fondo la pipeline causale in ambito grafico. Per ogni pixel e per ogni corpo della coppia, il kernel esegue il **doppio ritrovamento causale** ([§2.2](#22-come-la-fisica-interagisce-con-i-buffer)) per ottenere posizione e velocità *al tempo ritardato di quel pixel*, sottrae il moto del centro di massa, proietta la velocità ritardata sul versore pixel-sorgente e sulla sua ortogonale e mappa la differenza quadratica $v_r^2 - v_t^2$ (la formulazione fisica completa è nel [§7.6 della Guida alla Fisica](PHYSICS_AND_SCENARIO_GUIDE.md#76-deformazione-proiettata-gw-strain-quadrupolare)). Dal lato architetturale valgono due scelte:
...
```mermaid
flowchart TD
    A["Campione FPS del ciclo corrente"] --> B{"FPS sotto 30?"}
    B -->|"sì"| C["Downgrade immediato: stride<br/>raddoppiato<br/>gli FPS osservati finiscono in<br/>perf_memory"]
    B -->|"no"| D{"FPS sopra 58?"}
    D -->|"no"| E["Zona morta 30-58: nessuna azione"]
    D -->|"sì"| F{"3 cicli consecutivi sopra<br/>soglia<br/>e 5 secondi dall'ultimo<br/>downgrade?"}
    F -->|"no"| G["Attesa: streak non ancora maturata"]
    F -->|"sì"| H{"perf_memory: la risoluzione<br/>superiore<br/>era già stata provata sotto<br/>soglia?"}
    H -->|"sì"| I["Upgrade cancellato:<br/>si resta sulla configurazione<br/>corrente"]
...
Il moltiplicatore agisce sul **raggio di Schwarzschild** $R_s$ (l'orizzonte degli eventi, definizione nella [nomenclatura della Guida alla Fisica](PHYSICS_AND_SCENARIO_GUIDE.md#termini-essenziali-e-nomenclatura)), non sul raggio fisico. Non a caso si chiama *BH*_ACCRETION_MULT (BH = Black Hole): un gate a monte esclude i corpi ordinari, il cui confine di collisione resta il raggio fisico (`is_bh`, vero solo se $R_s$ supera lo 0,1% del raggio visivo). Il Sole, con $R_s \approx 3$ km contro 696.000 km di raggio, non lo attiverà mai: anche a 100× l'orizzonte espanso sarebbe 2.000 volte più piccolo della fotosfera. Il meccanismo tocca nella pratica solo buchi neri e stelle di neutroni.
...
La guardia EMRI (`emri_guard`) chiude questo buco, con due soglie invece di una sola per non sovra-espandere le coppie intermedie (l'espansione troppo aggressiva è essa stessa fonte di sovradissipazione, [§10.1 della Guida alla Fisica](PHYSICS_AND_SCENARIO_GUIDE.md#101-caso-di-studio-gw190814-la-sovradissipazione-in-campo-profondo)). Il rapporto di massa decide quanto espandere il raggio di cattura del corpo grande della coppia, sempre sotto la soglia $\sim1.9\times$ e solo a DT piccolo.
...
flowchart TD
    ...
    B -->|"no"| C["Pre-passo O(N): raggio di cattura<br/>per corpo<br/>(hitbox adattivo + guardia EMRI)"]
    C --> D{"Filtro spaziale per coppia:<br/>gap oltre max_move?"}
    D -->|"sì (oltre il 99% delle coppie)"| E["Early-exit sulla coppia"]
    D -->|"no"| F{"Ray cast CCD sul segmento<br/>del<br/>tick:<br/>intersezione con la sfera di<br/>cattura?"}
    F -->|"sì"| G["Fusione alla frazione di tick<br/>t_min interpolata"]
    F -->|"no"| H["Coppia salva per questo tick"]
    E --> I["Fine loop coppie"]
    G --> I
    H --> I
    I --> J["Calcolo del cooldown dinamico:<br/>minimo tra stima cinematica e<br/>tetto a 0,75c"]
```
...
```mermaid
flowchart TD
    A["Cattura eventi del sistema<br/>operativo (tastiera, mouse,<br/>finestra)"] --> B["event_handler (Interceptor Chain)<br/>Tutorial → Spawner → Faders →<br/>Console → Camera → Callbacks"]
    B --> C["engine.tick(steps): aggiornamento<br/>fisico ed integrazione causale"]
    C --> D["master_renderer: composizione<br/>sequenziale dei layer grafici<br/>[sfondo] → [heatmap] → [scie] →<br/>[corpi] → [UI/debug]"]
    D --> E["pygame.display.flip():<br/>aggiornamento schermo (eseguito<br/>una volta per frame)"]
```
...
4. **Scelta del punto specifico** (L1-L5): calcola posizione e velocità esatte con le stesse formule analitiche della Guida alla Fisica ([§9.4](PHYSICS_AND_SCENARIO_GUIDE.md#94-punti-di-lagrange-analitici-problema-dei-tre-corpi-circolare-ristretto) e [§9.5](PHYSICS_AND_SCENARIO_GUIDE.md#95-velocità-co-rotante-sui-punti-di-lagrange)); `get_ghost_markers()` espone le cinque posizioni per un'anteprima live disegnata a schermo prima della conferma.
